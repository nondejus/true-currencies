...
import {attributes} from '../scripts/registry';
...
describe('Staking', () => {
  ...
  describe('with Aave and AssuredFinancialOpportunity', () => {
    ...
    beforeEachWithFixture(async (provider, wallets) => {
      ...
      await registry.setAttributeValue(holder.address, attributes.whitelistTrueRewards, 1)

      await registry.subscribe(attributes.isRegisteredContract, trustToken.address)
      await registry.setAttributeValue(stakedToken.address, attributes.isRegisteredContract, 1)

      expect(await registry.subscriberCount(attributes.isRegisteredContract)).to.eq(1)
      expect(await registry.getAttributeValue(stakedToken.address, attributes.isRegisteredContract)).to.eq(1)
      expect(await registry.hasAttribute(stakedToken.address, attributes.isRegisteredContract)).to.be.true
    })
